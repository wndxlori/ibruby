...
VALUE connectServiceManager(VALUE self, VALUE user, VALUE password)
{
   ...
   if(manager->handle != 0)
   {
      rb_ibruby_raise(NULL, "Service manager already connected.");
   }
   ...
   *position++ = size;
                         ...
                         buffer))
   {
      free(buffer);
      free(service);
      rb_ibruby_raise(status, "Error connecting service manager.");
   }
...
VALUE disconnectServiceManager(VALUE self)
{
   ...
   if(manager->handle != 0)
   {
      ...
      {
         rb_ibruby_raise(status, "Error disconnecting service manager.");
      }
      manager->handle = 0;
   }
